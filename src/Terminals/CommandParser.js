...
const ParseCommand = (command, allPackages) => {
  ...
  allCommands.forEach((commandObj) => {
    if (commandObj.command === allPackages.commandSelector[0]) {
      ...
      if (allPackages.commandSelector.length === commandObj.arguments) {
        ...
        result = rejectCommand(
          allPackages.commandSelector.length,
  ...
  if (!foundCommand)
    result = [
      <p>{command} is not a recognized command</p>,
      <p>
...
const handleLS = (allPackages) => {
  ...
  respond.map((item) => {
    ...
    else return result.push(<p className="indented">{item.name}</p>);
  });
  result.push(<br />);
  return result;
};
...
const handleCD = (allPackages) => {
  ...
  if (newPath === "") result.push(<p>Can't open {commandSelector[1]}</p>);
...
const handleOPEN = (allPackages) => {
  ...
  if (!owners)
    ...
    result.push(
      <p>
        Permission denied owners: {owners.map((owner) => owner)}
        <br />⠀
      </p>
    );
...
const handleSU = (allPackages) => {
  ...
  result.push(
    <p>
      <b className="I">Password for {commandSelector[1]}:</b>*******
    </p>
  );
  result.push(
    <p className="I">
      Logged in as {commandSelector[1]}!<br />⠀
    </p>
  );
  ...
  let result = help_descriptions.map((commandObj) => {
    return (
      <p className="indented">
        <b className="I">[{commandObj.command}] </b>: {commandObj.description}
      </p>
    );
  });
  result.push(<br />);
...
const rejectCommand = (received, expected) => {
  return (
    <p>
      Incorrect Number of arguments. Received: {received}, expected: {expected}
      <br />⠀
    </p>
  );
};
